...
import floraEnvironment from '../util/floraEnvironment';
import floraConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!floraEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    floraConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!floraConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !floraConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (floraConfig.manageDaemonLifetime()) {
        floraEnvironment.startFloraDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Flora Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Flora-Network/flora-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Flora-Network/flora-blockchain/wiki/FAQ',
              );
            },
          },
          {
    ...
    if (process.platform === 'darwin') {
      // Flora Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Flora' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Flora Blockchain' }),
            click: () => {
              openAbout();
            },
          },
