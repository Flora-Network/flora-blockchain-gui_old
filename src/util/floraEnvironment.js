...
const getFloraVersion = () => {
  let version = null;
  const exePath = getExecutablePath('flora');
  // first see if we can get a flora exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or flora is on the path
    try {
      version = child_process
