...
global.daemon_rpc_ws = `wss://${self_hostname}:55422`;
...
function loadConfig(net) {
  try {
    // check if XFLCOIN_ROOT is set. it overrides 'net'
    const config_root_dir =
      'XFLCOIN_ROOT' in process.env
        ? process.env.XFLCOIN_ROOT
        : path.join(os.homedir(), '.flora', net);
    const config = yaml.load(
      fs.readFileSync(path.join(config_root_dir, 'config/config.yaml'), 'utf8'),
    );

    self_hostname = lodash.get(config, 'ui.daemon_host', 'localhost');

//    const daemon_port = lodash.get(config, 'ui.daemon_port', 55422); // hardcoded
    const daemon_port = '55422'; // hardcoded
