...
import { AlertDialog } from '@flora/core';
import {
  Typography,
  Button,
  Box,
...
import { flora_to_mojo } from '../../../util/flora';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      floraper_input.value === '' ||
      Number(floraper_input.value) === 0 ||
      !Number(floraper_input.value) ||
      isNaN(Number(floraper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const floraper = flora_to_mojo(floraper_input.value);
    const floraper_value = Number.parseInt(Number(floraper));
    const userpubkey = userpubkey_input.value;
    const amount = flora_to_mojo(amount_input.value);
    const amount_value = Number.parseInt(Number(amount));
    // var fee = flora_to_mojo(fee_input.value);
