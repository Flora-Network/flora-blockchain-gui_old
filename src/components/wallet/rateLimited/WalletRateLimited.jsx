...
import { mojo_to_flora_string, flora_to_mojo } from '../../../util/flora';
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const floraper_input = ip_parsed.limit;
    ...
    const floraper_value = Number.parseInt(Number(floraper_input));
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojo_to_flora_string(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = flora_to_mojo(fee_input.value);
