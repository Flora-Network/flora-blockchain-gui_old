...
function SendCard(props: SendCardProps) {
  ...
  function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      dispatch(
        openDialog(
          <AlertDialog>
            <Trans>
              Error: Cannot send flora to coloured address. Please enter a flora
              address.
            </Trans>
          </AlertDialog>,
        ),
    ...
    const amountValue = Number.parseFloat(flora_to_mojo(amount));
    const feeValue = Number.parseFloat(flora_to_mojo(fee));
