...
export default function Block() {
  ...
  const poolReward = mojo_to_flora(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_flora(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const floraFees = blockRecord.fees
    ? mojo_to_flora(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`http://explorer.floracoin.farm/search/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`http://explorer.floracoin.farm/search/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: floraFees ? `${floraFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
